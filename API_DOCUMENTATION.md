...
`IsIdentifyActive` (bool) turns identify mode on/off, flashing the device
LEDs so the physical unit can be located. Added to the official API docs in
the February 2025 update. This integration checks support at login and
exposes it as `switch.<name>_identify` only on devices that return the
object.
